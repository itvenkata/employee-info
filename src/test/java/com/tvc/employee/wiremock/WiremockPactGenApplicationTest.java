package com.tvc.employee.wiremock;
...
@SpringBootTest
class WiremockPactGenApplicationTest {
  ...
  void configureSystemUnderTest() {
    ...
    wireMockServer.addMockServiceRequestListener(
        WireMockPactGenerator.builder("the-consumer", "the-provider").build());
  ...
  void shouldEnsureThatServerWasStarted() {
    ...
    wireMockServer.addStubMapping(
        get(urlEqualTo("/posts/1")).willReturn(aResponse().withStatus(200)).build());

    // givenThat(get(urlEqualTo("/posts/1")).willReturn(aResponse().withStatus(200)));
    ResponseEntity<String> response =
        restTemplate.getForEntity("http://localhost:9090/posts/1", String.class);
